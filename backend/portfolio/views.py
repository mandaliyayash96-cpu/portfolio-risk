...
# This one WRITES, so it is the most urgent of the three to close.
# TODO Phase 4 (auth): IsAuthenticated, and scope the portfolio lookup to
#      request.user so nobody can add positions to somebody else's portfolio.
...
def holdings_import(request, portfolio_id: int):
    ...
    """
    return Response(import_holdings_csv(portfolio_id, request.FILES.get("file")))
...
def holding_detail(request, portfolio_id: int, holding_id: int):
    ...
    """
    return Response(delete_holding(holding_id, portfolio_id=portfolio_id))
